...
def fun(df,filename):
    ...
    for x in df['款号']:
        ...
        #print(saleSpuId)
        #print(spuId)
        if(len(saleSpuId) >= 30):  # 结果大于30个视为搜索失败
            print("没有该商品信息  款号"+x)
        else:
            ...
            print(str1.join(str(i) for i in imprice+discount))
            ...
            table.write(n, 2, str1.join(str(i) for i in imprice+discount))  # 最后一行追加数据
