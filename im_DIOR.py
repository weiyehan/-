...
def jsonprovide(spuid, price1):
    ...
    jasondata = {
        "ProductId": huohao1[0]+colorCode1[0],
        "Name": spuName1[0],
        "Price": price1,
        "saleVolume": "",
        "detail": descrip1[0]+coverimage1[0]+pinpai1[0]
    ...
    for x in df['款号']:
        ...
        if(len(saleSpuId) >= 50):  # 结果大于50个视为搜索失败
            ...
            for i in range(0, len(saleSpuId)):
                ...
                response = requests.post('http://imgateway.intramirror.com/imapp/spu/getSpuSkuInfo', headers=headers, data=json.dumps(spudata))
                
                if(str(getcolourcode(spuId1)).lower() == str(truecolourcode).lower()):#转化为小写比较
                    
                    count=count+1
                    price = re.findall(r"imPrice\"\:\"(.*?)\"", str(response.text))
                    discoun = re.findall(r"imDiscount\"\:\"(.*?)\"", str(response.text))
            ...
            table.write(n, 2, str1.join(str(i)for i in zip(imprice,discount)))  # 最后一行追加数据
