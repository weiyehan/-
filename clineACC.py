...
allurls = [
    'https://www.celine.com/zht-hk/celine-%E7%94%B7%E8%A3%9D/%E9%85%8D%E9%A3%BE/%E7%9A%AE%E5%B8%B6/']
...
for url0 in allurls:
    ...
    imgurls_xpaths = driver.find_elements_by_xpath(
        "/html/body/div[2]/main/div[2]/div/ul/li/div/a/div/div[1]/div[2]/img")
    nexturl_xpaths = driver.find_elements_by_xpath(
        "/html/body/div[2]/main/div[2]/div/ul/li/div/a")
    productids_xpaths = driver.find_elements_by_xpath(
        "/html/body/div[2]/main/div[2]/div/ul/li/div")
    ...
    try:
        ...
        for i in range(0, len(price_xpaths)):
            detail = ""
            try:
                ...
                imgurl1 = driver.find_element_by_xpath(
                    "//div[@class='o-product__imgs']/ul/li[1]//img").get_attribute("src")
                ...
                for s in size:
                    ...
                    sizes = sizes+" " + \
                        s.get_attribute("data-mformdd-attr-value")
                    if sizes==" TU":
                        sizes="均码"
                ...
                miaoshu = ""
                canshu = ""
                if re.search(r'CM', detail):
                    miaoshu = ""
                    canshu = re.findall(r'\n(.*?)CM', detail)[0]+"CM"
                if re.search(r'英寸', detail):
                    miaoshu = ""
                    canshu = re.findall(r'\n(.*?)英寸', detail)[0]+"英寸"
                jasondata = {
                ...
                with open(path+'\\'+filename, 'w', encoding='utf-8') as fp:
                    ...
                    #爬照片
                imgdata = requests.get(url=imgurl1.split(
                    "?")[0], headers=headers).content
                jpgname = productids[i]+"NO.1"+".jpg"
                with open(path+'\\'+jpgname, "wb") as f:
                    f.write(imgdata)
                imgdata = requests.get(url=imgurl2.split(
                    "?")[0], headers=headers).content
